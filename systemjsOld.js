...
(function () {
	...
	/**
	 * 解析非普通字符串或URL的相对路径
	 * @param {string} relUrl 相对URL
	 * @param {string} parentUrl 父级URL
	...
	function resolveIfNotPlainOrUrl(relUrl, parentUrl) {
		...
		// 如果相对URL以“//”开头，说明是协议相对URL，直接将父级URL的协议部分和相对URL拼接
		if (relUrl[0] === "/" && relUrl[1] === "/") {
			return parentUrl.slice(0, parentUrl.indexOf(":") + 1) + relUrl;
		}

		// relative URL
		// 如果相对URL以“./”、“../”或“/”开头，说明是相对URL，需要进行解析
		else if (
			(relUrl[0] === "." &&
				(relUrl[1] === "/" ||
					(relUrl[1] === "." &&
			...
			// 读取父级URL的路径名部分
			// 路径名部分是从第一个“/”开始到最后一个“/”结束的字符串
			var pathname;
			if (parentUrl[parentProtocol.length + 1] === "/") {
				// 如果父级URL的协议部分不是“file:”，则需要读取出授权和主机信息
				if (parentProtocol !== "file:") {
					pathname = parentUrl.slice(parentProtocol.length + 2);
					// 从路径名中去除前导的“/”
					pathname = pathname.slice(pathname.indexOf("/") + 1);
				} else {
					// 如果父级URL的协议部分是“file:”，则路径名部分从第8个字符开始
					pathname = parentUrl.slice(8);
				}
			} else {
				// 如果父级URL的协议部分是“:/”，则路径名部分从“/”开始
				pathname = parentUrl.slice(
					parentProtocol.length + (parentUrl[parentProtocol.length] === "/")
				);
			}

			// 如果相对URL以“/”开头，说明是从根目录开始的相对路径，直接将父级URL的协议和主机部分与相对URL拼接
			if (relUrl[0] === "/")
				return (
					parentUrl.slice(0, parentUrl.length - pathname.length - 1) + relUrl
				);

			// 将路径名和相对URL拼接，并分割成数组以便处理“..”和“.”段
			// 为了性能考虑，使用循环而不是其他高级方法
			// “../../../z”相对于“x/y”解析为“z”
			...
			// 如果最后一个字符不是“/”，将其作为一个新的段添加到输出数组中
			if (segmentIndex !== -1) output.push(segmented.slice(segmentIndex));

			// 将父级URL的协议和主机部分与处理后的路径名部分拼接，得到最终的绝对URL
			return (
	...
	function resolveAndComposePackages(
		...
		for (var p in packages) {
			...
			console.log(resolvedLhs, "resolvedLhs???", p);
			var rhs = packages[p];
			console.log(rhs, "rhs???", resolveIfNotPlainOrUrl(rhs, baseUrl));
