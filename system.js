...
(function () {
	...
	function SystemJS() {
		this[REGISTRY] = {};
        this.importMapPromise = Promise.resolve();
        this.importMap = {
            imports: {},
            scopes: {},
            depcache: {},
	...
	systemJSPrototype.addImportMap = function (newMap, mapBase) {
		resolveAndComposeImportMap(newMap, mapBase || baseUrl, this.importMap);
	};
		...
		[].forEach.call(document.querySelectorAll("script"), function (script) {
			...
			if (script.type === "systemjs-module") {
				...
				var fetchPromise = script.src
					...
					: script.innerHTML;
				this.importMapPromise = this.importMapPromise
					.then(function () {
						return fetchPromise;
					})
					.then(function (text) {
						extendImportMap(this.importMap, text, script.src || baseUrl);
					});
			}
		});
	}
	...
	systemJSPrototype.createScript = function (url) {
		...
		var integrity = this.importMap.integrity[url];
	...
	systemJSPrototype.instantiate = function (url, firstParentUrl) {
		...
		var loader = this;
		return Promise.resolve(this.createScript(url)).then(function (
			script
		) {
	...
	systemJSPrototype.instantiate = function (url, parent, meta) {
		...
		return this.fetch(url, {
			...
			integrity: this.importMap.integrity[url],
			meta: meta,
		}).then(function (res) {
			if (!res.ok)
				throw Error(
	...
	systemJSPrototype.instantiate = function (url, firstParentUrl, meta) {
		var preloads = this.importMap.depcache[url];
		if (preloads) {
